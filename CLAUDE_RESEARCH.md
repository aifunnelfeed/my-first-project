д# Research Engine — М.И.С. (Маркетинговая Исследовательская Система)
...
**DR (Deep Researcher) — особые правила:**
- НИКОГДА не запускается автоматически — только по явному подтверждению пользователя
- При DATA_CONFIDENCE LOW → предложить пользователю вариант "E) Deep Research по {категория}"
- Пользователь выбирает тему, глубину (15/30/50 итераций) и подтверждает запуск
- Результат DR интегрируется в collection.md → повторный DATA_CONFIDENCE_CHECK
- Также доступен как standalone команда: `/deep-research {тема}`
