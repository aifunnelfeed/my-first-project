...
Если proof недостаточно — пометить "нужно подтвердить" И указать план: (A) запросить у пользователя, (B) найти через research, (C) использовать hypothesis с пометкой [ТРЕБУЕТ ПОДТВЕРЖДЕНИЯ].
