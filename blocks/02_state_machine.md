...
**ALTERNATIVES SCOUT (фоновый, параллельно с KB GATE):**
После показа PHASE A пользователю → dispatch J (Alternatives Scout) в фоне по протоколу [DISPATCH_RULES_V1] (блок 11, секция 7.3):
- J работает параллельно с KB GATE и PHASE B — НЕ блокирует основной поток
- Контекст: brief-context + research-context + strategy v1 (секции A-K)
- Результат: alternatives.md (кумулятивно)
- Краткая сводка показывается пользователю ПОСЛЕ завершения STRATEGY (вместе с или после вопроса "Стратегия 1 или 2?")
...
6. (Опционально) ✅ Alternatives Scout (J) завершился → alternatives.md сохранён (НЕ блокирующее условие — если J ещё работает, переход разрешён)

ЗАПРЕТ: Переход STRATEGY → EXECUTION без выполнения ВСЕХ 5 обязательных условий = нарушение протокола. Молчаливый bypass любого из условий запрещён.
...
### РУЧНЫЕ КОМАНДЫ (доступны в любом стейте)

**"Альтернативы" / "Alternatives" / "Альтернативы для {блок}":**
- Dispatch J (Alternatives Scout) по шаблону 7.14 из блока 11
- Контекст адаптируется по текущему стейту (таблица в DISPATCH_RULES_V1, блок 11, секция 7.3)
- Результат показывается пользователю сразу + сохраняется в alternatives.md (кумулятивно)
- Пользователь может выбрать альтернативу для интеграции ("Используй альтернативу #N") или сохранить на потом
- Git commit: `alt({slug}): manual — {описание}`
- Fallback: если Agent tool недоступен → главный агент генерирует альтернативы in-context (сокращённый набор: 1 Big Idea + 2 Headlines + 1 Enhancement)
...
**АРХИТЕКТУРА:** Два параллельных потока — субагент KB-Scout усиливает поиск основного агента.
...
1. Попробуй `search_knowledge(query, category)` через MCP по категориям `formulas` и `examples` для текущего блока
...
- По возвращении скаута: объединить KB-SCOUT REPORT + свои находки → показать пользователю ПЕРЕД написанием блока
- Если скаут вернул TOOL_UNAVAILABLE → использовать только свои результаты
- Основной агент принимает финальное решение о релевантности (у него полный контекст проекта)

**ЗАПРЕТ:** Молчаливое игнорирование knowledge/ = нарушение протокола. Либо используй (через скаут + свой поиск), либо получи явный отказ пользователя.

**PROOF USAGE REGISTRY (инициализация):**
При старте EXECUTION (первый блок PHASE C) — создай секции "Proof Usage Registry" и "Placeholder Registry" в state.md (пустые таблицы). После каждого блока COPY — обнови оба реестра: добавь все proof-элементы, использованные в блоке, и все вставленные плейсхолдеры. Перед написанием каждого нового блока — сверься с Proof Usage Registry для anti-repetition (по [PLACEHOLDER_SYSTEM_V1], блок 07, секция 4.9g).
...
**FALLBACK:** Если Agent tool недоступен → 29 вопросов in-context (блок 09, секция 6.1)

— 1-3 правки (если FAIL): "Причина → Правка"
— запрещено генерировать новый блок
— после PASS: "Принять блок? (да / правки / следующий блок)"

### PLACEHOLDER RESOLUTION (между последним DEBUGGING и DELIVERY)

**TRIGGER:** Все обязательные блоки прошли DEBUGGING (PASS или OVERRIDE). Перед сборкой final.md.

**ПРОТОКОЛ:**
1. Прочитай Placeholder Registry в state.md → собери все OPEN плейсхолдеры
2. Если OPEN = 0 → переходи к DELIVERY
3. Если OPEN > 0 → покажи пользователю:

```
⚠️ ОТКРЫТЫЕ ПЛЕЙСХОЛДЕРЫ ({кол-во}):

| # | Тип | Что нужно | Блок |
|---|-----|-----------|------|
| P1 | {тип} | {описание} | {блок} |

Варианты:
A) Заполнить сейчас — предоставьте данные для каждого пункта
B) Убрать — удалить плейсхолдеры и перестроить текст вокруг имеющихся proof
C) Оставить как [ВСТАВИТЬ:...] — вы заполните после получения final.md
```

4. **Вариант A:** Пользователь предоставляет данные → вставить в текст → пометить RESOLVED → повторный DEBUGGING блока
5. **Вариант B:** Убрать плейсхолдер → перестроить текст → повторный DEBUGGING блока
6. **Вариант C:** Пометить в state.md: `PLACEHOLDER_OVERRIDE: {кол-во} placeholders accepted by user` → перейти к DELIVERY с предупреждением

### STATE=DELIVERY

**PLACEHOLDER GATE (перед сборкой):**
Перед сборкой final.md проверь Placeholder Registry в state.md:
